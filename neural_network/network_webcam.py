...
from loadData_webcam import load_data
import random
...
X_train, y_train, X_test, y_test = load_data(path, ['d'])
...
model.add(Convolution2D(64, (3, 3), input_shape=(1, height, width), padding='same', activation='relu', kernel_constraint=maxnorm(3)))
...
model.add(Convolution2D(64, (3, 3), padding='same', activation='relu', kernel_constraint=maxnorm(3)))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Dropout(0.27))
